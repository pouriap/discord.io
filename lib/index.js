...
function handleWSMessage(data, flags) {
	...
	switch (message.t) {
		...
		case "MESSAGE_CREATE":
			client.emit('message', _data.author.username, _data.author.id, _data.channel_id, _data.content, _data.id, message);
			emit(client, message, _data.author.username, _data.author.id, _data.channel_id, _data.content, _data.id);
...
function writeToLog(text){
	...
	log += '--------------------';
	log += '\r';
	fs.appendFile('error_log.txt', log, function(){});
}